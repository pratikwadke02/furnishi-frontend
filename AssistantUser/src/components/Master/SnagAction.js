...
const TABLE_HEAD = [
  { id: 'snagActionCode', label: 'Snag Action Code', alignRight: true },
  { id: 'snagAction', label: 'Action', alignRight: true },
];
...
const SnagAction = (props) => {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = snagActionsTable.map((n) => n.id);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - snagActionsTable.length) : 0;

  const filteredUsers = applySortFilter(snagActionsTable, getComparator(order, orderBy), filterName);

  const isUserNotFound = filteredUsers.length === 0;
  
    const [snagAction, setSnagAction] = useState({
      action:'',
    ...
    const handleSubmit = async (e) => {
      ...
      try {
        ...
        alert("snagAction submitted successfully");
      } catch (error) {
        console.log(error);
      }
    };
    ...
    return (
      <>
        <form onSubmit={handleSubmit}>
            {/* <Box mt={2}>
            <Typography variant="h6">Snag Action</Typography>
          </Box> */}
          {/* <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, mb: 2 }}>
            ...
            <FormControl fullWidth sx={{ width: '100%', ml: { md: 1 }, mt: { xs: 2, md: 0 } }}>
              ...
              <Select
                labelId="demo-simple-select-label"
                id="demo-simple-select"
                value={snagAction.clientProductCode}
                label="Product Code"
                onChange={(e) => {
                  setsnagAction({
                    ...snagAction,
          ...
          <TableContainer sx={{ minWidth: 800 }}>
            <Table>
              <UserListHead
                ...
                rowCount={snagActionsTable.length}
                numSelected={selected.length}
                onRequestSort={handleRequestSort}
                onSelectAllClick={handleSelectAllClick}
              />
              <TableBody>
                {snagActionsTable.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((custInfo) => {
                  const { id, actionCode, action } =
                    custInfo;
