...
import NewSnaglist from '../components/snaglist/NewSnaglist';
import SnaglistOpen from '../components/snaglist/SnaglistOpen';
import SnaglistClosed from '../components/snaglist/SnaglistClosed';
...
const Snaglist = () => {
  ...
  const snaglists = (useSelector((state) => state.snaglist.snaglists));
  const statuses = (useSelector((state) => state.status.statuses));
  const issues = (useSelector((state)=> state.snagIssue.snagIssues));
  const actions = (useSelector((state)=> state.snagAction.snagActions));
  const solutions = (useSelector((state)=> state.snagSolution.snagSolutions));
  const costs = (useSelector((state)=> state.snagCost.snagCosts));
  console.log('snaglists', snaglists);
          ...
          <Card sx={{ p: 2 }}>
            <Box>
              ...
              <TabPanel value={headTab} index={0}>
                <NewSnaglist cordinators={cordinators} products={products} statusActions={statusActions} statuses={statuses} actions={actions} costs={costs} solutions={solutions} issues={issues} />
              </TabPanel>
              <TabPanel value={headTab} index={1}>
                <SnaglistOpen snaglists={snaglists} openModal={handleOpenModal} />
              </TabPanel>
              <TabPanel value={headTab} index={2}>
                <SnaglistClosed snaglists={snaglists} openModal={handleOpenModal} />
              </TabPanel>
            </Box>
          </Card>
        </Container>
