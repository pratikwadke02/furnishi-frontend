...
const TABLE_HEAD = [
  { id: 'snagIssueCode', label: 'Snag Issue Code', alignRight: true },
  { id: 'issue', label: 'Issue', alignRight: true },
];
...
const SnagIssue = (props) => {
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = snagIssuesTable.map((n) => n.id);
  ...
  const emptyRows = page > 0 ? Math.max(0, (1 + page) * rowsPerPage - snagIssuesTable.length) : 0;

  const filteredUsers = applySortFilter(snagIssuesTable, getComparator(order, orderBy), filterName);
          ...
          <TableContainer sx={{ minWidth: 800 }}>
            <Table>
              <UserListHead
                ...
                rowCount={snagIssuesTable.length}
                numSelected={selected.length}
                onRequestSort={handleRequestSort}
                onSelectAllClick={handleSelectAllClick}
              />
              <TableBody>
                {snagIssuesTable.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((custInfo) => {
                  const { id, issueCode, issue } =
                    custInfo;
